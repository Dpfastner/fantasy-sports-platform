...
| **Phase 46b** | Signup & Onboarding Friction Reduction | **Conversion-critical.** Competitive analysis (April 2026, vs. my_golf_pool Masters tool) revealed our signup flow is a 5-10 minute experience vs. competitor's ~30-second pick-and-go. Options to explore: (1) Guest Play Mode — submit picks with name + localStorage, claim via magic link later. (2) SSO signup (Google/Apple, skip email verify). (3) Hybrid public share link — commissioner shares quick-pick URL, users submit with just a name, claim with account later. Goal: reduce time-to-first-pick from ~5 minutes to <90 seconds. Also simplify landing when joining via invite link, remove unnecessary gates (age verification one-click), streamline first-time pool join. Priority: tackle before next major event (March Madness 2027 / CFB 2026). | Year 1 |
