...
━━━ UPCOMING ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Phase 40 (Dashboard Redesign) ▓▓▓▓░░░░░░░░  In progress
Phase 41 (AI Support Agent)  ░░░░░░░░░░░░  Ticket system + Claude triage
...
| **Phase 40** | Unified Dashboard Redesign | **Header**: Sleeper-inspired minimal header — logo only, no nav text links. Responsive hamburger menu for mobile. **Dashboard**: Merge leagues + events into single unified view. Cards for all active games (leagues, brackets, pick'ems, survivor) with live status badges. Quick-action buttons (make picks, view bracket, check scores). **Toast audit**: Standardize all feedback messages to use single Toast component (bottom-right). Remove inconsistent patterns (top banners with auto-scroll, inline success messages, `window.confirm` dialogs). **Navigation**: Streamline page transitions, consistent back buttons. | Mar 2026 |
| **Phase 41** | AI Support Agent | **41a**: Enhance existing `issue_reports` into ticket system with conversation threads (`ticket_responses` table), user-facing `/support` pages, admin ticket detail with reply UI. **41b**: Claude AI integration — auto-triage on submission (priority, category, summary), admin-triggered suggested responses, FAQ knowledge base search. Uses `@anthropic-ai/sdk` with Sonnet for cost efficiency. Claude never auto-responds — admin reviews and approves all suggestions. Migration: 054_ticket_responses.sql. | Mar 2026 |
