...
export function OpenChatButton({ channel }: OpenChatButtonProps) {
  ...
  return (
    <button
      onClick={handleOpen}
      className="flex items-center gap-2 px-4 py-3 bg-surface border border-brand/30 rounded-lg text-sm text-text-secondary hover:bg-brand/20 hover:text-text-primary transition-colors w-full"
    >
